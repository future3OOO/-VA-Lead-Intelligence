...
2. **Fetch**: Each adapter calls its public API or scoped web crawl, governed by `RateLimiter` and `Budget`. For example, the `openstreetmap` adapter queries the public Overpass API for real ANZ business listings by category.
...
7. **Enrich**: `enrich_contact_routes()` persists routes found by the source, `company_web`, or `hunter_domain` after qualification.
