...
async def list_company(
    ...
    result = await session.scalars(
        select(DBCompany)
        .where(DBCompany.workspace_id == auth_workspace_id)
        .offset(skip)
        .limit(limit)
    )
...
async def create_company(
    ...
    record = DBCompany(**values, workspace_id=auth_workspace_id)
    session.add(record)
...
async def get_company(
    ...
    record = await session.scalar(
        select(DBCompany).where(
            DBCompany.id == company_id, DBCompany.workspace_id == auth_workspace_id
        )
    )
    if not record:
