...
async def list_campaign_run(
    ...
    result = await session.scalars(
        select(DBCampaignRun)
        .where(DBCampaignRun.workspace_id == auth_workspace_id)
        .offset(skip)
        .limit(limit)
    )
...
async def create_campaign_run(
    ...
    record = DBCampaignRun(**values, workspace_id=auth_workspace_id)
    session.add(record)
...
async def get_campaign_run(
    ...
    record = await session.scalar(
        select(DBCampaignRun).where(
            DBCampaignRun.id == campaign_run_id, DBCampaignRun.workspace_id == auth_workspace_id
        )
    )
    if not record:
