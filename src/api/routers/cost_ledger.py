...
async def list_cost_ledger(
    ...
    result = await session.scalars(
        select(DBCostLedger)
        .where(DBCostLedger.workspace_id == auth_workspace_id)
        .offset(skip)
        .limit(limit)
    )
...
async def create_cost_ledger(
    ...
    record = DBCostLedger(**values, workspace_id=auth_workspace_id)
    session.add(record)
...
async def get_cost_ledger(
    ...
    record = await session.scalar(
        select(DBCostLedger).where(
            DBCostLedger.id == cost_ledger_id, DBCostLedger.workspace_id == auth_workspace_id
        )
    )
    if not record:
